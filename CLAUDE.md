...
- **Phân quyền**: Spatie Laravel Permission (teams mode, department-based, guard `web`)
...
- `tree(Request)` → dữ liệu cây (departments, permissions, post-categories)
...
/**
 * @group Core - Role
 * @header X-Department-Id ID đơn vị (bắt buộc với endpoint auth). Example: 1
 *
...
- Endpoint yêu cầu auth: `@header X-Department-Id`
...
- Spatie Permission với **Teams mode ON**, team column = `department_id`
...
```
Request → auth:sanctum → set.permissions.team (đọc X-Department-Id) → log.activity → permission:{resource}.{action},web → Controller
```
...
**Resource labels hiện có**: users=người dùng, posts=bài viết, post-categories=danh mục bài viết, permissions=quyền, roles=vai trò, departments=đơn vị, documents=văn bản, document-types=loại văn bản, issuing-agencies=cơ quan ban hành, issuing-levels=cấp ban hành, document-signers=người ký, document-fields=lĩnh vực, settings=cấu hình hệ thống, log-activities=nhật ký truy cập
...
```
admin@example.com / 123123        → Super Admin (toàn quyền, Thường trực Thành ủy)
thuky@example.com / 123123        → Thư ký (lập lịch cho Lãnh đạo, Thường trực Thành ủy)
tonghop@example.com / 123123      → Công chức tổng hợp (điều chỉnh tất cả lịch, Văn phòng Thành ủy)
canbo@example.com / 123123        → Cán bộ công chức (lập lịch của mình, Văn phòng Thành ủy)
```
