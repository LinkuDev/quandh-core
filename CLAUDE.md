...
- **Phân quyền**: Spatie Laravel Permission (teams mode, organization-based, guard `web`)
...
**Modules hiện có**: Auth, Core, Post, Document
...
- `tree(Request)` → dữ liệu cây (organizations, permissions, post-categories)
...
/**
 * @group Core - Role
 * @header X-Organization-Id ID tổ chức (bắt buộc với endpoint auth). Example: 1
 *
...
- Endpoint yêu cầu auth: `@header X-Organization-Id`
...
- Spatie Permission với **Teams mode ON**, team column = `organization_id`
...
```
Request → auth:sanctum → set.permissions.team (đọc X-Organization-Id) → log.activity → permission:{resource}.{action},web → Controller
```
...
**Resource labels hiện có**: users=người dùng, posts=bài viết, post-categories=danh mục bài viết, permissions=quyền, roles=vai trò, organizations=tổ chức, documents=văn bản, document-types=loại văn bản, issuing-agencies=cơ quan ban hành, issuing-levels=cấp ban hành, document-signers=người ký, document-fields=lĩnh vực, settings=cấu hình hệ thống, log-activities=nhật ký truy cập
...
```
admin@example.com / quandcore**11  → Super Admin (toàn quyền)
basic@example.com / quandcore**11  → Vai trò mẫu (chỉ xem)
```
