...
- **Phân quyền**: Spatie Laravel Permission (global mode, guard `web`)
...
- Spatie Permission với **Teams mode OFF** (global permissions)
...
```
Request → auth:sanctum → set.permissions.team (đồng bộ guard web) → log.activity → permission:{resource}.{action},web → Controller
```
