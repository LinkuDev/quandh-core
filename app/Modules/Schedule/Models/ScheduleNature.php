...
/**
 * Danh mục tính chất cuộc họp (catalog pattern).
 */
class ScheduleNature extends Model
{
    use HasFactory;
    ...
    {
        static::creating(fn (ScheduleNature $model) => $model->created_by = $model->updated_by = auth()->id());
        static::updating(fn (ScheduleNature $model) => $model->updated_by = auth()->id());
    }
    ...
    {
        return $this->hasMany(Schedule::class, 'nature_id');
    }
